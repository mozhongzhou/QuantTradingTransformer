...
def get_raw_data_path(stock_code=""):
    ...
    return os.path.join(raw_dir, f"{stock_code}_daily.csv" if stock_code else "")
...
def setup_logger():
    ...
    log_file = os.path.join(log_dir, "download_daily_data.log")
...
def download_stock_data(stock, start_date, end_date, interval, save_dir):
    """
    下载单只股票日 K 线数据并保存到CSV文件
    :param stock: 股票代码
    :param start_date: 开始日期
    :param end_date: 结束日期
    :param interval: 数据间隔（固定为 1d）
    :param save_dir: 保存目录
    """
    try:
        logger.info(f"开始下载 {stock} 日 K 线数据（{start_date} 至 {end_date}）")
        
        data = yf.download(
            tickers=stock,
            start=start_date,
            end=end_date,
            interval=interval,  # 固定为 "1d"
            progress=False,
            auto_adjust=True
        )
        
        if data.empty:
            logger.warning(f"{stock} 未找到日 K 线数据，跳过保存")
            return
        
        save_path = os.path.join(save_dir, f"{stock}_daily.csv")
        data.to_csv(save_path, encoding="utf-8")
        logger.info(f"{stock} 日 K 线数据已保存至 {save_path}")
        
    except Exception as e:
        logger.error(f"下载 {stock} 日 K 线数据时出错: {str(e)}")
